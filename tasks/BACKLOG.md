...
- [~] (go/ts) Card attachment: slots, class grants, discard-on-loss rules.
  ...
  - [~] Apply attached effects in resolution: granted slots, recycle draw/hand modifiers, ICE-face / ICE-dice / Black-ICE modifiers, and `space.modifier.kind = ice` now resolve in both mirrors. Attachment-granted movement now executes through explicit movement-option selection with card / once-per-turn activation. Armor replacing defense dice/nullifying ICE, granted stealth semantics, and the remaining `abilityUses` modes (`perTurn`, `dice`) remain pending.
