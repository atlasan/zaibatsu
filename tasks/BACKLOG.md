...
- [~] (go/ts) Ability resolution (**T-104**): Search, Delete, Reboot, Icebreaker.
  ...
  - [x] Multi-target Delete (split dice across co-located targets) + area attacks.
    ...
    - [x] Area attacks: Bomb-class area Delete and typed block `area-attack` effects now resolve against every pawn in the affected block with one shared roll per die. Identical across mirrors.
- [x] (go/ts) Bonus fragments → bonus icons → bonus counters; card costs paid in bonus counters.
- [x] (go/ts) Typed source-backed effect subset: typed block/card `place-pawn` now executes in both mirrors for the current authored Speedrunners slice (`Idoru`, `Chiba City`, `Cracker`) with structured `pawn-placed` events and `/play/` coverage.
- [~] (go/ts) Card attachment: slots, class grants, discard-on-loss rules.
  ...
  - [~] Apply attached effects in resolution: granted slots, recycle draw/hand modifiers, ICE-face / ICE-dice / Black-ICE modifiers, `space.modifier.kind = ice`, attachment-granted movement, armor-style defense replacement (`defenseOverride`), ICE nullification, and granted stealth reporting now resolve in both mirrors. Granted movement options support explicit selection plus card / once-per-turn / `perTurn` / `d6` move-use budgets. Broader non-move `abilityUses` handling remains pending.
