...
- [~] (go/ts) Card attachment: slots, class grants, discard-on-loss rules.
  ...
  - [~] Apply attached effects in resolution: granted slots, recycle draw/hand modifiers, ICE-face / ICE-dice / Black-ICE modifiers, `space.modifier.kind = ice`, and attachment-granted movement now resolve in both mirrors. Granted movement options support explicit selection plus card / once-per-turn / `perTurn` / `d6` move-use budgets. Armor replacing defense dice/nullifying ICE, granted stealth semantics, and broader non-move `abilityUses` handling remain pending.
