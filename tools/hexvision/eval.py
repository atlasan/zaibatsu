...
def evaluate_truth(build_dir: str = DEFAULT_BUILD, truth_path: str = BLOCKS_TRUTH) -> list[dict]:
    ...
    for asset, exp in truth.get("blocks", {}).items():
        ...
        rows.append({
            ...
            "bonusCorners": [sum(1 for c in tile.bonusCorners if c), exp.get("bonusCorners")],
            ...
            "iceModZones": [(d["zone"], d["face"]) for d in tile.iceDiceCandidates if d.get("role") == "modifier"],
        })
    return rows
    ...
    if truth:
        ...
        for r in truth:
            print(f"  {r['name']}: edgesOpen={r['edgesOpen'][0]}/{r['edgesOpen'][1]} "
                  ...
                  f"iceDice={r['iceDice']} (iceValue={r['iceValue']}) iceCorner={r['iceCorner']} iceMod={r['iceModZones']}")
    return 0
