...
export interface EditorSpace {
  ...
  /** Physical printed zones owned by this gameplay space. */
  zoneIds: ZoneId[];
  /** Positive finite capacity, or unlimited. New finite spaces default to zoneIds.length. */
  capacity: SpaceCapacity;
  /** Mandatory source-review explanation when capacity differs from the default. */
  capacityNote?: string;
  /** Derived source-geometry neighbours, exported for later step movement. */
  neighbors: string[];
  pawnId?: string;
  effectId?: string;
...
export function validateDocument(document: EditorDocument, assets: AssetRecord[], layout?: BlockLayout): string[] {
  ...
  if (document.resourceType === "action-card") {
    ...
    if ((document.status === "review" || document.status === "verified") && !card.summary?.trim()) errors.push("Reviewed cards require a concise gameplay summary.");
  ...
  if (!block.name.trim()) errors.push("Block name is required.");
  if (!blockAssets.has(document.source.assetId) || !block.assetRefs.includes(document.source.assetId)) errors.push("Block assetRefs must include the selected block asset.");
  ...
  if (block.layoutId !== STANDARD_BLOCK_LAYOUT_ID) errors.push("Blocks must use the standard seven-zone layout.");
  ...
  if (document.status === "verified" && zoneOwner.size !== STANDARD_ZONE_IDS.length) errors.push("A verified block must account for all seven physical zones.");
  return errors;
}
...
export function normalizeBlockDocument(document: BlockDocument, layout: BlockLayout): BlockDocument {
  ...
  const spaces = rawSpaces.map((raw) => {
    ...
    return { id: raw.id, type, zoneIds, capacity, ...(raw.capacityNote ? { capacityNote: raw.capacityNote } : {}), neighbors: [], ...(raw.pawnId ? { pawnId: raw.pawnId } : {}), ...(raw.effectId ? { effectId: raw.effectId } : {}), ...(raw.modifier ? { modifier: raw.modifier } : {}) };
  });
  return { ...document, block: { ...document.block, layoutId: STANDARD_BLOCK_LAYOUT_ID, spaces: inferNeighbors(spaces, layout) } };
}
