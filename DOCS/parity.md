...
| Card attachment | ✅ | ✅ | attach to pawn/enemy/block, slot + cost checks, discard+refund on elimination/takeover; granted slots, recycle draw/hand modifiers, ICE-face / ICE-dice / Black-ICE modifiers, and explicit attachment-granted movement activation with card / once-per-turn / `perTurn` / `d6` move-use budgets now resolve identically |
